...
import { BriefcaseBusiness, Gauge, Rocket, Server, Code } from "lucide-react";
...
export default function AchievementsSection() {
  ...
  return (
    <section className="section-shell py-24 sm:py-32">
      ...
      <div className={`grid gap-4 md:grid-cols-2 ${metrics.length === 4 ? "lg:grid-cols-4" : "lg:grid-cols-3"}`}>
        {metrics.map(({ icon, value, suffix, label, detail, emoji }, index) => {
          ...
          return (
            <motion.article key={label} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .3 }} transition={{ delay: index * .1 }} whileHover={{ y: -9, rotate: index === 1 ? -1 : 1 }} className="relative overflow-hidden rounded-2xl border border-border bg-card p-7 text-left transition hover:border-accent-gold/30 hover:shadow-lg">
              <div className={`absolute -right-7 -top-7 h-28 w-28 rounded-full blur-2xl ${index % 3 === 0 ? "bg-accent-gold/[.04] dark:bg-accent-gold/[.08]" : index % 3 === 1 ? "bg-accent-rose/[.04] dark:bg-accent-rose/[.08]" : "bg-accent-emerald/[.04] dark:bg-accent-emerald/[.08]"}`} />
              <Icon className={`relative ${index % 3 === 0 ? "text-accent-gold" : index % 3 === 1 ? "text-accent-rose" : "text-accent-emerald"}`} size={21} />
              <p className="relative mt-8 text-5xl font-bold tracking-[-.05em] text-slate-900 dark:text-slate-100"><CountUp value={value} suffix={suffix} /><span className="ml-2 text-lg">{emoji || "⚡"}</span></p>
